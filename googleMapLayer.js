...
(function (tangelo, $, google) {
    ...
    if (!($ && $.widget && google)) {
        tangelo.GoogleMapLayer = tangelo.unavailable({
            ...
            required: ["JQuery", "JQuery UI", "Google Maps API"]
        });
        return;
    }

    tangelo.widget("tangelo.googleMap", {
        options: {
            latitude: tangelo.accessor({value: 0}),
            longitude: tangelo.accessor({value: 0}),
            zoom: tangelo.accessor({value: 2})
        },
        ...
        _create: function () {
            ...
            mapOpts = {};
            mapOpts.zoom = mapOpts.zoom();
            mapOpts.center = new google.maps.LatLng(this.options.latitude(), this.options.longitude());
